...
fn test_highlighting_doesnt_break_formatting() {
    ...
    let location = CodeFrameLocation {
        start: Location {
            ...
            line: 1,
            column: Some(10), // Exclusive: marks "foo"
        }),
    };
    ...
    // Render WITH highlighting but force_color=false so we get plain output
    ...
    // With force_color=false, both should be identical
    assert_eq!(
...
#[test]
fn test_multibyte_highlighting() {
    // Verify that syntax highlighting works correctly with multi-byte source.
    // Token boundaries from SWC are byte-based; they must align with char boundaries
    // since tokens can't split multi-byte chars.
    let source = "const 名前 = 'こんにちは';";
    ...
    // Render with highlighting enabled, then strip ANSI and verify structure
    let options_highlighted = CodeFrameOptions {
        force_color: true,
        highlight_code: true,
        ..Default::default()
    };
    let result = render_code_frame(source, &location, &options_highlighted).unwrap();
    let stripped = strip_ansi_codes(&result);

    // Render without highlighting for comparison
    let options_plain = CodeFrameOptions {
        force_color: false,
        highlight_code: false,
        ..Default::default()
    ...
    // Stripped highlighted output should match plain output
    assert_eq!(
        stripped, result_plain,
        "Highlighted output with ANSI stripped should match plain output for multi-byte source"
    );
}
